...
public class AdminTablesView extends VerticalLayout {
    ...
    public AdminTablesView(StudyTableService tableService, RoomService roomService, SecurityService securityService) {
    ...
    private void openDialog(StudyTable table) {
        ...
        dialog.setHeaderTitle(table.getId() == null ? "Nuovo Tavolo" : "Modifica Tavolo");
        ...
        if (table.getCapacity() > 0) capacityField.setValue(table.getCapacity());
        if (table.getRoom() != null) roomSelect.setValue(table.getRoom());

        Button save = new Button("Salva", e -> {
            if (roomSelect.getValue() == null || numberField.getValue() == null) {
                Notification.show("Compila tutti i campi!"); return;
            }
            try {
                table.setNumber(numberField.getValue());
                table.setCapacity(capacityField.getValue() != null ? capacityField.getValue() : 0);
